...
# Duplicate of deserializer.rb, except:
#  @ -> $ (retain caches across multiple deserializations)
#  TODO - could add a reset feature for multiple FLM's
...
module RocketAMF
  module Pure
    ...
    class AMF3Deserializer
      def initialize
        $string_cache = [] if $string_cache == nil
        $object_cache = [] if $object_cache == nil
        $trait_cache = []  if $trait_cache == nil
      end
